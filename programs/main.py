...
from utils import get_lectures_between_times  # Fixed duplicate import
from attendance import mark_attendance_for_lectures
...
while True:
        ...
        if entry_mode:
            ...
            if name in entry_times:
                if name not in exit_times or (current_timestamp - exit_times[name] > cooldown_period):
                    ...
                    mark_attendance_for_lectures(name, entry_time, current_time)
    ...
    if key != 255:  # Check if any key is pressed (255 means no key was pressed)
        print(f"Key pressed: {chr(key)}")  # Debugging line to see which key is pressed
